...
function atualizarFiltrosAplicados() {
  ...
  const { textoBusca, emEstoque, foraEstoque, categorias } =
    estado.filtrosAtivos;
  if (textoBusca) {
    elementos.filtrosAplicados.appendChild(
      criarTagFiltro(`Busca: ${escapeHtml(textoBusca)}`, () => {
  ...
  if (foraEstoque) {
    elementos.filtrosAplicados.appendChild(
      criarTagFiltro("Fora de estoque", () => {
...
function aplicarFiltros(resetarPagina = true) {
  ...
  const produtosFiltrados = estado.produtos.filter((produto) => {
    ...
    if (categorias.length > 0) {
      ...
      const encontrou = categorias.some((cat) =>
...
function resetarFiltros() {
  ...
  elementos.statusEstoque.checked = false;
  elementos.statusForaEstoque.checked = false;
  elementos.categoriaCheckboxes.forEach(
    (cb) => (cb.checked = cb.id === "cat-todos")
  );
  atualizarFiltros();
}
