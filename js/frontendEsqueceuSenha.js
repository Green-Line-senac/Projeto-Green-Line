...
async function handleRecuperacao(e) {
  ...
  try {
    ...
    if (window.authModern) {
      window.authModern.setButtonLoading('enviar-recuperacao', true, 'spinner-only');
    } else {
...
async function fazerRequisicaoRecuperacao(email) {
  return await fetch(`${api.online}/enviar-email`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ 
      email: email,
      assunto: "Redefinir senha - GreenLine", 
      tipo: "recuperacao", 
      pedido: null 
    }),
  });
...
function processarRespostaRecuperacao(dados, email) {
  if (dados.conclusao === 2) {
    // Mostrar mensagem de sucesso com instruções de redefinição
    mostrarMensagemHTML(email);
    ...
    mostrarMensagem(
      "Não foi possível enviar as instruções. Verifique se o email está correto.", 
      "error"
    );
  }
}
...
function mostrarMensagemHTML(email) {
  if (window.authModern) {
    ...
    paragrafo2.textContent = '🔑 Verifique sua caixa de entrada e clique no link para criar uma nova senha.';
    ...
    paragrafo3.textContent = 'Não encontrou o email? Verifique a pasta de spam.';
    ...
    paragrafoContador.innerHTML = '⏰ Você será redirecionado para o login em <span id="countdown-reset">10</span> segundos';
...
function mostrarMensagemCancelada() {
  if (window.authModern) {
    ...
    const paragrafo1 = document.createElement('p');
    paragrafo1.style.marginBottom = '10px';
    paragrafo1.textContent = 'Enviamos um link para redefinir sua senha para seu email.';
